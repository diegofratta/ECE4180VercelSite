...
const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate file using the validateFile utility
    const validationResult = validateFile(selectedFile);

    if (!validationResult.valid) {
      // Log validation error with file context
    ...
    // Use setTimeout to ensure state is updated before upload starts
    setTimeout(() => {
    ...
    try {
      ...
      if (!presignedUrlResponse.ok) {
        ...
        if (presignedUrlResponse.status === 403) {
          throw new Error('You do not have permission to upload images. Staff access required.');
  ...
  const handleButtonClick = () => {
    // Always trigger file selection - upload happens automatically on file select
